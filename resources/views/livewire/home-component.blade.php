                ...
                <div class="wrap-product-tab tab-style-1">						
                    <div class="tab-contents">
                        <div class="tab-content-item active" id="digital_1a">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                @foreach($letestProduct as $letproduct)
                                    <div class="product product-style-2 equal-elem ">
                                        <div class="product-thumnail">
                                            <a href="{{route('product.details',$letproduct->slug)}}" title="{{$letproduct->name}}">
                                                <figure><img src="{{ asset('assets/images/products')}}/{{$letproduct->image}}" width="800" height="800" alt="{{$letproduct->name}}"></figure>
                                            </a>
                                            <div class="group-flash">
                                                <span class="flash-item new-label">new</span>
                                            </div>
                                            <div class="wrap-btn">
                                                <a href="{{route('product.details',$letproduct->slug)}}" class="function-link">quick view</a>
                                            </div>
                                        </div>
                                        <div class="product-info">
                                            <a href="{{route('product.details',$letproduct->slug)}}" class="product-name"><span>{{$letproduct->name}}</span></a>
                                            <div class="wrap-price"><span class="product-price">${{$letproduct->regular_price}}</span></div>
                                        </div>
                                    </div>
                                @endforeach
